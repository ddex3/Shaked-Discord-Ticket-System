...
function infoRow(label: string, value: string): string {
  ...
  const prefix = `  ${c.white}${B}${label}${R} ${dots} ${c.cyan}${value}${R}`;
...
export function printStartup(client: Client): void {
  ...
  console.log(border("┌", "─", "┐", c.cyan));
  console.log(centerLine(`${c.cyan}${B}Shaked Ticket System${R}`, c.cyan));
  console.log(border("└", "─", "┘", c.cyan));
  ...
  console.log(infoRow("Commands", `${cmds.length} loaded`));
  console.log(border("└", "─", "┘", c.gray));

  console.log();
  console.log(statusLine(`Online as ${c.cyan}${B}${tag}${R}`));
  console.log(statusLine(`Serving ${c.cyan}${B}${guilds}${R}${c.white} guild${guilds !== 1 ? "s" : ""} with ${c.cyan}${B}${users.toLocaleString()}${R}${c.white} members`));
  console.log(statusLine(`${c.cyan}${B}${cmds.length}${R}${c.white} slash commands registered`));
  console.log();
...
export function printDeployHeader(): void {
  ...
  console.log(border("┌", "─", "┐", c.cyan));
  console.log(centerLine(`${c.cyan}${B}Shaked Ticket System${R}`, c.cyan));
  console.log(centerLine(`${c.gray}${D}Command Deployer${R}`, c.cyan));
  console.log(border("└", "─", "┘", c.cyan));
  console.log();
}

export function printDeployProgress(count: number): void {
  console.log(`  ${c.yellow}${B}⟳${R} ${c.white}Registering ${c.cyan}${B}${count}${R}${c.white} slash commands...${R}`);
}

export function printDeploySuccess(count: number, target: string): void {
  console.log(`  ${c.green}${B}✓${R} ${c.white}Successfully registered ${c.cyan}${B}${count}${R}${c.white} commands ${c.gray}${D}(${target})${R}`);
  console.log();
  console.log(`  ${c.gray}${D}Developed by ${c.white}${B}Shaked Angel${R}  ${c.gray}${D}·  github.com/ddex3${R}`);
  console.log();
}
...
export function printDeployError(error: unknown): void {
  console.log(`  \x1b[31m${B}✗${R} ${c.white}Failed to register commands${R}`);
  console.error(error);
}
